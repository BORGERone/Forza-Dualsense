...
def kill_processes_on_port(port: int) -> None:
    """Kill all processes that are using the specified port."""
    import os
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            # Check if this is our program (python with main.py or FH-DualSense.exe)
            cmdline = proc.info['cmdline']
            if cmdline:
                cmdline_str = ' '.join(cmdline)
                if 'main.py' in cmdline_str or 'FH-DualSense.exe' in cmdline_str:
                    log.info(f"Killing process {proc.info['name']} (PID {proc.info['pid']}) using port {port}")
                    proc.kill()
                    killed = True
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            pass
    if killed:
        log.info(f"Port {port} cleared successfully")
        import time
        time.sleep(0.5)  # Wait for OS to release the port
...
class UDPListener:
    ...
    def __enter__(self):
        ...
        # Retry binding if port is still in use
        max_retries = 10
        for attempt in range(max_retries):
            try:
                ...
                if hasattr(e, 'winerror') and e.winerror == 10048 and attempt < max_retries - 1:
                    log.warning(f"Port {self.port} still in use, killing old processes and retrying in 0.5s... (attempt {attempt + 1}/{max_retries})")
